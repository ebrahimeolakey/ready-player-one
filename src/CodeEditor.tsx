...
function guides(width:number,theme:AppearanceValue["theme"]) {
...
function minimap(theme:AppearanceValue["theme"]) {
 return ViewPlugin.fromClass(class {
  ...
  draw(){
   ...
   const doc=this.view.state.doc,rows=Math.max(1,Math.floor(height)),stride=Math.max(1,Math.ceil(doc.lines/rows));context.fillStyle=theme==='light'?'#64778a':'#86988d';
...
export function editorSettingsExtensions(settings:EditorSettingsValue,theme:AppearanceValue["theme"]="dark"):Extension[] {
 ...
 if(settings.indentGuides)extensions.push(guides(settings.indentWidth,theme));
