...
export function CollaborationPanel({state,session:s,call,onOpenComment}:{state:State;session:Session;call:Call;onOpenComment?:(comment:Session['comments'][number])=>void}) {
 const [plan,setPlan]=useState(''),[comment,setComment]=useState(''),[path,setPath]=useState(''),[releaseId,setReleaseId]=useState(''),[releaseNote,setReleaseNote]=useState('');
 ...
 const members=[...new Map(state.members.filter(m=>(!m.workspaceId||m.workspaceId===s.workspaceId)&&(!m.sessionId||m.sessionId===s.id)).map(m=>[m.id,m])).values()];
 ...
 {s.plan.map(p=><div className={'plan-row '+(p.done?'completed':'')} key={p.id}>
  ...
  {!p.assigneeId&&canComment&&<button onClick={()=>call('plan.claim',{...params,id:p.id})}>领取</button>}{canComment&&p.assigneeId===state.me?.id&&<button onClick={()=>{setReleaseId(p.id);setReleaseNote('');}}>归还</button>}</div>
 {releaseId===p.id&&<form className="compact-input" onSubmit={async e=>{e.preventDefault();if(await call('plan.release',{...params,id:p.id,note:releaseNote}))setReleaseId('');}}><input aria-label="归还备注" placeholder="归还备注…" value={releaseNote} onChange={e=>setReleaseNote(e.target.value)} required/><button disabled={!releaseNote.trim()}>归还</button><button type="button" onClick={()=>setReleaseId('')}>取消</button></form>}
 {(p.history||[]).some(h=>h.action==='released')&&<details className="help-details"><summary>归还记录</summary>{p.history?.filter(h=>h.action==='released').map((h,i)=><p key={h.at+':'+i}><small>{h.actor} · {time(h.at)}</small><br/>{h.note}</p>)}</details>}
 ...
 {s.comments.map(c=><article key={c.id} className={c.status==='resolved'?'resolved':''}><header><strong>{c.owner}</strong><time>{time(c.at)}</time></header>{c.anchor&&(c.location||c.transcript)?<button className="comment-anchor" onClick={()=>onOpenComment?.(c)}>{c.anchor}{c.stale?' · 代码已更新':''}</button>:c.anchor&&<small>{c.anchor}</small>}<p>{c.text}</p>
