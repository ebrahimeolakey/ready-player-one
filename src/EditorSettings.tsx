...
export function EditorSettings({settings,call}:{settings?:Partial<EditorSettingsValue>;call:Call}) {
 ...
 useEffect(()=>{const value=JSON.parse(savedSignature);setDraft(value);setNumbers({fontSize:String(value.fontSize),lineHeight:String(value.lineHeight),indentWidth:String(value.indentWidth)});},[savedSignature]);
 ...
 async function save(value?:EditorSettingsValue){setBusy(true);setMessage('');try{if(!value&&Object.values(numbers).some(v=>!v.trim()))throw Error('请填写字号、行高和缩进宽度');value??={...draft,fontSize:Number(numbers.fontSize),lineHeight:Number(numbers.lineHeight),indentWidth:Number(numbers.indentWidth)};const result=await call('settings.editor.save',{settings:validateEditorSettings(value)});if(result){setDraft(resolveEditorSettings(result));setMessage('已保存');}}catch(error){setMessage((error as Error).message);}finally{setBusy(false);}}
