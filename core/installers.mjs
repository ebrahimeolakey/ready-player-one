...
export function assetSpec(id, arch = process.arch) {
...
export async function installTool(
  ...
  if (process.platform !== "darwin") throw Error("当前安装助手仅支持 macOS");
  await mkdir(dir, { recursive: true, mode: 0o700 });
  const temp = await mkdtemp(join(dir, ".install-"));
  try {
    if (id === "claude") {
      onProgress("正在下载 Anthropic 官方安装程序……");
      const script = await getURL("https://claude.ai/install.sh", {
        limit: 1e6,
      });
      ...
      await exec("/bin/bash", [path], {
        env: localEnv(),
        timeout: 240000,
        maxBuffer: 2e6,
      });
      return { command: "claude", source: "https://claude.ai/install.sh" };
    }
    ...
    if (!asset) throw Error("没有找到适用于此 Mac 的官方安装包");
    if (
    ...
    const { stdout } = await exec("/usr/bin/tar", ["-tf", archive]);
    if (
      stdout
        .split("\n")
    ...
    await exec("/usr/bin/tar", ["-xf", archive, "-C", extracted], {
      timeout: 30000,
    });
