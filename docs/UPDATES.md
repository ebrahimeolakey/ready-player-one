...
- **Mac ZIP**：检查归档路径、应用 bundle ID、版本和 Mach-O 架构；复制到应用目录旁的新位置；用户确认后由独立脚本等待当前进程退出，保留旧应用备份、原位替换并重新启动。替换或启动命令失败时恢复旧应用。不依赖用户安装 Xcode。临时挂载的 App Translocation 应用先要求移入应用程序目录。
...
Mac 官方包实际网络下载和校验结果由本轮验证记录补充。安装替换测试只操作临时目录，没有升级或替换用户当前安装的应用。完整安装包重启和 Linux 实机更新仍需对应平台验收。
