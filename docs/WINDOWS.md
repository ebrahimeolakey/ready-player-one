...
适用于 **Windows 10 / 11 x64**。当前版本 0.4.4-beta.1，与 Mac 同版；Amoeba 功能补齐仍在开发。

## 安装

- 安装器：运行 `Ready-Player-One-0.4.4-beta.1-windows-x64-setup.exe`，按提示安装到当前用户目录。
...
0.4.4 已在 GitHub Windows 2022 runner 上原生构建 ZIP / NSIS，并通过 28 项回归、真实 Electron 启动、加密设置、PowerShell PTY 输入输出/尺寸/取消/退出清理，以及合成 Provider CLI 的真实 ConPTY 检查。源码提交和产物 SHA256 均已核对，见 [Windows 原生 CI 证据](WINDOWS-NATIVE-CI.md)。
