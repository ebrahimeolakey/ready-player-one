...
## 后续源码中的健康协议（未替代上述历史实测）

后续源码已新增 Mac 安装后双阶段健康确认、精确 bundle/进程围栏和失败恢复，见 [UPDATE-HEALTH.md](./UPDATE-HEALTH.md)。只有旧、新两包均声明相同受支持代际时才启用；上述 0.3/0.4 发布包没有声明，因此不据此声称这次历史升级已经覆盖自动健康回滚。Windows/Linux 也不继承 Mac 验收结论。
