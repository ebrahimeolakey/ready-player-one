...
2026-09-24 本批完整测试 211 项通过；TypeScript / Vite 构建通过。桌面图片链路实际添加 64×64 纯蓝 PNG，发起只读审批，经本机 Codex 返回“蓝色”。这是实际账户调用，并非模拟回复。同一会话双分栏输入已验证双向同步。

用户确认先发未签名测试版，Apple Developer 证书、GitHub OAuth 应用与固定 HTTPS 回调配置以后补。已发布版本仍是 0.4.0-beta.1；本节源码改动不能当作现有安装包已包含。
