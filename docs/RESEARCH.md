...
v0.2 已补充本机官方账号授权、GitHub 仓库接入和 Cloudflare 临时公网中继。v0.3 逐页重新观察实际应用，并读取布局尺寸 / 颜色参数进行中文 UI 重排；范围及差异见 [界面对照](UI-REFERENCE.md)。上方「尚未达到」列表是最初版本记录，以最新 README 和界面对照为准。

## 2026-09-24 功能差距复核

最新逐项对照见 [与 Amoeba 的功能差距](AMOEBA-GAPS.md)。该表以 0.1.38 官方发布日志和当前源码为准，区分已证实的原版能力与尚未核实的能力；不再将企业 SSO、完整 Provider 内部 session 跨机器迁移等未经证实的功能直接列作原版已有。
