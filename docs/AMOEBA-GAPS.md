...
上表保留 0.3.0-beta.1 历史基线。2026-09-24 后续开发已补活动文件 TTL、明确文件/目录范围、计划路径与步骤关联、实际分支输入、工作区隔离和结构化冲突解释，详见 [重叠提醒的规则与边界](OVERLAP-DETECTION.md)。相邻任务判断使用明确标注的关键词规则，不能据此称为语义 AI 或与原版内部算法完全相同；UI 与真实分支采集由桌面接入完成。
