...
0.4.3 进度：A2 结构化额度/限流待接管、A3 未知结果证据与人工确认、A4 本机 MCP 子任务桥、A7 真正取消搜索已实现，完整回归 277/277 通过。对应文档列明自动化/桌面证据和边界。

0.4.4 进度：A5 消息/本机 diff 锚点、自动引用检查与记忆历史，A6 会话限定邀请/归还/工作区删除，A8 独立本机 CLI，A9 原文与摘要分离已实现；隔离源码 319/319 回归通过。A7 活跃文件聚合、完整视觉对照和 B 类真实环境验收继续保留。


0.4.5 进度：A7 未过期活动聚合、应用内 GitHub 创建/绑定、编辑器设置及 Mac 双阶段健康检查已实现，隔离全套 361/361 通过。真实 Electron 健康门禁确认/拒绝与编辑器交互有独立证据。B6 的已发布包实际升级仍待验收；原版通用偏好新增实测缺口见 [实测补充](UI-OBSERVATION-2026-09-24.md)。

0.4.6 进度：通用布局/密度/空编辑器、分类通知、托盘与提示音、启动更新检查已实现，隔离全套 375/375 通过。Apple Silicon 真实旧版 UI 下载并升级 0.4.5→0.4.6，双阶段确认、包摘要/备份和设置保留通过，见 [发行包升级](PUBLISHED-UPGRADE-0.4.6.md)。B6 的其他平台、版本组合和实际失败回滚仍保留。
...
| **B6：新版本 UI 更新闭环** | 隔离数据目录与应用副本，从真实已发布旧 UI 检查并安装后续测试版 | digest、旧包备份、新包哈希、进程重启和配置保留；[已有 Mac 实验](UPDATER-MAC-VALIDATION.md)已验证真实替换/启动，但不是 0.3 UI 点击更新。0.4.5 已实现兼容声明一致时的健康检测/失败恢复，并验证真实 main/React 门禁；Apple Silicon 上 0.4.5→0.4.6 已通过实际公开包 UI 下载/替换/确认及设置保留；其他平台/版本组合、真实失败回滚仍待验收 |
...
深浅主题、协作者颜色、VS Code 兼容编辑设置/快捷键的选择文件→预览→勾选→加密保存入口已接入。导入不会覆盖同页未保存的其他字段；失效预览和冲突拒绝，保存失败恢复旧设置。401 项隔离回归通过；Mac 实际安装包验证原生选择器和导入结果，Windows 原生 CI 另验证 28 项平台、22 项导入及外观保存/重启。详见 [外观](APPEARANCE.md)、[导入](VSCODE-IMPORT.md)、[平台记录](WINDOWS-NATIVE-CI.md)。

仍不能把本批当成通用设置全部完成：启动恢复、聊天编辑器标签、审查按钮位置、成员在缩略图中的写入位置、提示重置和扩展迁移不在 0.4.7 包内。后续工作树代码与发布版本分别验收。
