...
| **B6：新版本 UI 更新闭环** | 隔离数据目录与应用副本，从真实已发布旧 UI 检查并安装后续测试版 | digest、旧包备份、新包哈希、进程重启和配置保留；[已有 Mac 实验](UPDATER-MAC-VALIDATION.md)已验证真实替换/启动，但不是 0.3 UI 点击更新。0.4.5 已实现兼容声明一致时的健康检测/失败恢复，并验证真实 main/React 门禁；仍需实际发行包替换端到端验收 |
